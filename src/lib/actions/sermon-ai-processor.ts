...
import { extractTranscript } from '@/lib/services/transcript-extractor'
...
async function processPipeline(
  ...
  const validation = validateMediaLink(url)
  if (!validation.valid) {
    throw new Error(validation.error || 'Invalid link format')
  }

  // Step 5: Update log to 'processing', extract transcript
  ...
  // Step 9: Return ProcessingResult with success=true and draft data
  return {
    success: true,
    draft: {
      title: seo.title,
