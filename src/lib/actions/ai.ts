...
import { normalizeAiSeoOutput } from '@/lib/seo/normalize-ai-seo-output'
...
SEO SCORING (target 100/100 — match the admin SEO panel checks exactly):
- focusKeyword: 2–4 words; MUST appear inside seoTitle, inside metaDescription, in the first 200 characters of html, and inside slug (+15 pts each when correct).
- seoTitle: length MUST be 50–60 characters (count before returning). Include focusKeyword verbatim (+15 pts).
- metaDescription: length MUST be 150–160 characters (count before returning). Include focusKeyword (+15 pts).
- html: at least 300 words of readable text; focusKeyword in the first 200 characters (+15 pts each).
- slug: lowercase, hyphenated; MUST contain slugified focusKeyword (e.g. "worship arts" → worship-arts) (+15 pts).
- excerpt: 100–150 characters for cards/listings.
- featuredImage: not generated by AI — user uploads separately (+10 pts).

VERIFY before returning JSON: count seoTitle.length (50–60), metaDescription.length (150–160), and plain-text word count of html (≥300). Adjust wording until counts fit.
`
  ...
  try {
    ...
    for (const modelName of MODEL_CHAIN) {
      try {
        ...
        const parsed = parseResilientJson(text)
        return normalizeAiSeoOutput(parsed, {
          title: req.title,
          requestedKeyword: req.focusKeyword,
        })
