...
export default async function MinistryDetailPage({ params }: Props) {
  ...
  const ministryJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'CommunityGroup',
    name: ministry.name,
    description: ministry.description || `Learn about ${ministry.name} at Kingdom Deliverance Centre Uganda.`,
    url: `https://kdcuganda.org/ministries/${ministry.slug}`,
    ...(ministry.image_url ? { image: ministry.image_url } : {}),
    parentOrganization: {
      '@type': 'Church',
  ...
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(ministryJsonLd) }}
      />
      <BreadcrumbSchema items={generateBreadcrumbs('ministry', ministry.name, ministry.slug)} />
